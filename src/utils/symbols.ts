...
const exchangeDefinitions: ExchangeDefinition[] = [
  ...
  { code: "NSE", name: "National Stock Exchange of India", country: "IN", region: "asia", market: "securities", assetTypes: ["equity", "etf", "index"], aliases: ["NSEINDIA"], providerSymbols: { yahoo: { suffix: ".NS" }, stooq: { suffix: ".NS" } }, defaultProviders: ["yahoo", "stooq", "news"] },
  { code: "BSE", name: "Bombay Stock Exchange", country: "IN", region: "asia", market: "securities", assetTypes: ["equity", "etf", "index"], aliases: [], providerSymbols: { yahoo: { suffix: ".BO" }, stooq: { suffix: ".BO" } }, defaultProviders: ["yahoo", "stooq", "news"] },
  ...
  { code: "SZSE", name: "Shenzhen Stock Exchange", country: "CN", region: "asia", market: "securities", assetTypes: ["equity", "etf", "index"], aliases: ["SHZ"], providerSymbols: { yahoo: { suffix: ".SZ" }, stooq: { suffix: ".CN" } }, defaultProviders: ["yahoo", "stooq", "news"] },
  { code: "ASX", name: "Australian Securities Exchange", country: "AU", region: "asia", market: "securities", assetTypes: ["equity", "etf", "index"], aliases: ["AUX"], providerSymbols: { yahoo: { suffix: ".AX" }, stooq: { suffix: ".AU" } }, defaultProviders: ["yahoo", "stooq", "news"] },
  { code: "TSX", name: "Toronto Stock Exchange", country: "CA", region: "americas", market: "securities", assetTypes: ["equity", "etf", "index"], aliases: ["TSE_CA"], providerSymbols: { yahoo: { suffix: ".TO" }, stooq: { suffix: ".CA" } }, defaultProviders: ["yahoo", "stooq", "news"] },
...
const yahooExchangeAliases: Record<string, string> = {
  NMS: "NASDAQ",
  NCM: "NASDAQ",
  NGM: "NASDAQ",
  NYQ: "NYSE",
...
export function listExchanges(): ExchangeDefinition[] {
...
export function getExchange(exchange: string): ExchangeDefinition {
  ...
  return (
    exchangeMap.get(normalized) ?? {
      ...
      aliases: [],
      providerSymbols: { yahoo: {}, stooq: {} },
      defaultProviders: ["yahoo", "stooq", "news"]
    }
  );
}

export function normalizeExchange(exchange?: string): string | undefined {
  return exchange ? getExchange(exchange).code : undefined;
}
...
export function providerSymbol(symbol: string, exchange: ExchangeDefinition, provider: ProviderName, assetType?: AssetType): string {
  ...
  if (provider === "yahoo" && assetType === "forex") {
    return cleaned.endsWith("=X") ? cleaned : `${cleaned}=X`;
  }

  if (provider === "yahoo") {
    const suffix = YAHOO_EXCHANGE_SUFFIX[exchange.code];
    if (suffix && !cleaned.endsWith(suffix) && !cleaned.startsWith("^")) {
      return `${cleaned}${suffix}`;
    }
  }

  if (provider === "stooq" && ["NASDAQ", "NYSE", "AMEX"].includes(exchange.code) && !cleaned.includes(".") && !cleaned.startsWith("^")) {
    return `${cleaned}.US`.toLowerCase();
  }
...
export function normalizeInstrument(exchange: string, symbol: string, assetType?: string): NormalizedSymbol {
  ...
  const providerSymbols = Object.fromEntries(
    definition.defaultProviders.map((provider) => [provider, providerSymbol(cleaned, definition, provider, resolvedAssetType)])
  ...
  return {
    ...
    providerSymbols,
    canonical: `${definition.code}:${cleaned}`
  };
}
...
export function splitSymbols(symbols: string, exchange = "NASDAQ"): string[] {
  return symbols
    .split(",")
    .map((symbol) => normalizeInstrument(exchange, symbol).providerSymbols.yahoo ?? symbol.trim().toUpperCase())
    .filter(Boolean);
}
...
export function exchangeFromYahoo(exchange?: string): string | undefined {
  return exchange ? yahooExchangeAliases[exchange.toUpperCase()] ?? normalizeExchange(exchange) : undefined;
}

export function assetTypeFromYahoo(type?: string): AssetType | undefined {
  return type ? yahooTypeMap[type.toUpperCase()] : undefined;
}
